...
class ServiceController extends Controller
{
    public function index(){
        $query_allServices = Service::AllUserId(auth()->id());
        $query_date_now = Service::forUser(auth()->id(), 'now');
        $query_date_next = Service::forUser(auth()->id(), 'next');
        return response()->json([
            'message' => 'servicios',
            'data' => ['servicesCalendar'=> $query_allServices, 'services_act' => $query_date_now, 'services_next' => $query_date_next],
        ], 200);
    }
